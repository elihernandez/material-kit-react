...
import { Template } from '../../../recursoiglesia/api/models/Template'
...
const Page = () => {
    ...
    const [selectedData, setSelectedData] = useState<Template>()
    ...
    const getData = async () => {
        const response = await axios.get(`${API_URL}/plantillas/all`)
    ...
    const handleChangeSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        newData.data = search === ''
            ? templates
            : newData.data = data.data.filter((template: Template) =>
                template.name.toLowerCase().includes(search.toLowerCase()) ||
                template.link.toLocaleLowerCase().includes(search.toLowerCase())
            )

        setData(newData)
    }

    const handleOpenModal = (d: Template, type: string) => {
        setSelectedData(d)
    ...
    return (
        <>
            <Head>
                <title>
                    Plantillas | Panel Recurso Iglesia
                </title>
            </Head>
            <Box
                component="main"
            ...
            >
                <Container maxWidth={false}>
                    <Toolbar title="Plantillas" handleChange={handleChangeSearch} handleOpenCreateModal={handleOpenCreateModal} />
                    <Box sx={{ mt: 3 }}>
                        {data &&
...
const CreateFormModal = ({ open, handleClose, getData, handleOpenSnackbar }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await axios.post(`${API_URL}/plantillas`, {
                name: name,
                imgUrl: imgUrl,
                url: url,
                link: link
    ...
    return (
        <Dialog open={open} onClose={handleClose} fullWidth maxWidth='sm'>
            <form onSubmit={handleSubmit}>
                <DialogTitle>Información de plantilla</DialogTitle>
                <DialogContent>
                    <DialogContentText>
...
const EditFormModal = ({ open, handleClose, data, getData, handleOpenSnackbar }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await axios.put(`${API_URL}/plantillas`, {
                id: id,
                name: name,
                imgUrl: imgUrl,
                url: url,
    ...
    return (
        <Dialog open={open} onClose={handleClose} fullWidth maxWidth='sm'>
            <form onSubmit={handleSubmit}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Editar información de plantilla
                    </DialogContentText>
...
const DeleteFormModal = ({ open, handleClose, data, getData, handleOpenSnackbar }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await axios.delete(`${API_URL}/plantillas`, {
                data: {
                    id: data.id
                }
            })
