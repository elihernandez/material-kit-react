...
const Page = () => {
    ...
    const [selectedData, setSelectedData] = useState<ResourceDownload>()
    const [downloads, setDownloads] = useState<Array<ResourceDownload>>([])
    ...
    const getData = async () => {
        const response = await axios.get(`${API_URL}/descarga/all`)

        setDownloads(response.data)
        setData({
            headers: [
                { name: 'id' }, { name: 'nombre' }, { name: 'fecha' }
            ],
            data: response.data
        })
    }
    ...
    const handleChangeSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        newData.data = search === ''
            ? downloads
            : newData.data = data.data.filter((download: ResourceDownload) =>
                download.resourceId.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const handleOpenModal = (d: ResourceDownload, type: string) => {
        setSelectedData(d)
    ...
    return (
        <>
            <Head>
                <title>
                    Descargas | Panel Recurso Iglesia
                </title>
            </Head>
            <Box
                component="main"
            ...
            >
                <Container maxWidth={false}>
                    <Toolbar title="Descargas" handleChange={handleChangeSearch} handleOpenCreateModal={handleOpenCreateModal} />
                    <Box sx={{ mt: 3 }}>
                        {data &&
                        ...
                        <TableBody>
                            {data.data.slice(page * limit, page * limit + limit).map((d) => (
                                <TableRow hover key={d.id} selected={selectedCustomerIds.indexOf(d.id) !== -1}>
                                    ...
                                    <TableCell style={{ width: 600 }} component="th" scope="row">
                                        {d.name}
                                    </TableCell>
