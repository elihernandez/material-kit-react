...
const Page = () => {
    ...
    const [selectedData, setSelectedData] = useState<Shortener>()
    const [shorteners, setShorteners] = useState<Array<Shortener>>([])
    ...
    const getData = async () => {
        const response = await axios.get(`${API_URL}/acortador/all`)
    ...
    const handleChangeSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        newData.data = search === ''
            ? shorteners
            : newData.data = data.data.filter((shortener: Shortener) =>
                shortener.name.toLowerCase().includes(search.toLowerCase()) ||
                shortener.link.toLocaleLowerCase().includes(search.toLowerCase())
            )

        setData(newData)
    }

    const handleOpenModal = (d: Shortener, type: string) => {
        setSelectedData(d)
        ...
        <>
            ...
            >
                <Container maxWidth={false}>
                    <Toolbar title="Enlaces" handleChange={handleChangeSearch} handleOpenCreateModal={handleOpenCreateModal} />
                    <Box sx={{ mt: 3 }}>
                        {data &&
