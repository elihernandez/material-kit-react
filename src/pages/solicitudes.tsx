...
import { Shortener } from '../../../recursoiglesia/api/models/Shortener'
...
const Page = () => {
    ...
    const [selectedData, setSelectedData] = useState<MultitrackRequest>()
    const [requests, setRequests] = useState<Array<MultitrackRequest>>([])
    ...
    const getData = async () => {
        const response = await axios.get(`${API_URL}/solicitudes/all`)

        setRequests(response.data)
        setData({
            headers: [
                { name: 'id' }, { name: 'nombre' }, { name: 'album' }, { name: 'artista' },
                { name: 'email' }, { name: 'fecha' }, { name: 'Envíado' }
            ],
            data: response.data
        })
    }
    ...
    const handleChangeSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        newData.data = search === ''
            ? requests
            : newData.data = data.data.filter((request: MultitrackRequest) =>
                request.multitrack.name.toLowerCase().includes(search.toLowerCase()) ||
                request.multitrack.album.name.toLocaleLowerCase().includes(search.toLowerCase()) ||
                request.multitrack.album.artist.name.toLocaleLowerCase().includes(search.toLowerCase())
            )

        setData(newData)
    }

    const handleOpenModal = (d: MultitrackRequest, type: string) => {
        setSelectedData(d)
    ...
    return (
        <>
            <Head>
                <title>
                    Solicitudes | Panel Recurso Iglesia
                </title>
            </Head>
            <Box
                component="main"
            ...
            >
                <Container maxWidth={false}>
                    <Toolbar title="Solicitudes" handleChange={handleChangeSearch} handleOpenCreateModal={handleOpenCreateModal} />
                    <Box sx={{ mt: 3 }}>
                        {data &&
                    ...
                    <EmailFormModal open={openEmailModal} handleClose={handleCloseEmailModal} data={selectedData} getData={getData} handleOpenSnackbar={handleOpenSnackbar} />
                    <Snackbar
                        ...
                        <TableBody>
                            {data.data.slice(page * limit, page * limit + limit).map((d) => (
                                <TableRow hover key={d.id} selected={selectedCustomerIds.indexOf(d.id) !== -1}>
                                    ...
                                    <TableCell style={{ width: 150 }} component="th" scope="row">
                                        {d.multitrack.album.name}
                                    </TableCell>
                                    <TableCell style={{ width: 150 }} component="th" scope="row">
                                        {d.multitrack.album.artist.name}
                                    </TableCell>
                                    ...
                                    <TableCell style={{ width: 100 }} component="th" scope="row">
                                        <Chip label={`${d.isSent ? 'Enviado' : 'Pendiente'}`} color={`${d.isSent ? 'primary' : 'warning'}`} />
                                    </TableCell>
                                    <TableCell style={{ width: 50 }} component="th" scope="row">
                                        <Stack direction="row">
                                            ...
                                            <IconButton aria-label="edit" onClick={() => handleOpenModal(d, 'email')}>
                                                <ForwardToInboxIcon />
                                            </IconButton>
                                        </Stack>
                                    </TableCell>
...
const EmailFormModal = ({ open, handleClose, data, getData, handleOpenSnackbar }) => {

    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()

        try {
            await axios.post(`${API_URL}/solicitudes/email`, {
                data: {
                    ...data
                }
            })
            ...
            handleOpenSnackbar({
                type: 'error',
                message: 'Ocurrió un problema, no se envió el email.'
            })
        }
    }
    ...
    return (
        <Dialog open={open} onClose={handleClose} fullWidth maxWidth='sm'>
            <form onSubmit={handleSubmit}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Se va a envíar un email de solicitud de recurso: <br /><br />
                        SongId: {data?.songId} <br />
                        Nombre: {data?.multitrack.name} <br />
                        Álbum: {data?.multitrack.album.name} <br />
                        Artista: {data?.multitrack.album.artist.name} <br />
                        Email: {data?.email} <br />
                        Fecha: {new Date(data?.createdAt).toLocaleDateString()} <br />
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleClose}>Cancelar</Button>
