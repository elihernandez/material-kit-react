...
const Page = () => {
    ...
    const [selectedData, setSelectedData] = useState<Multitrack>()
    const [multitracks, setMultitracks] = useState<Array<Multitrack>>([])
    ...
    const getData = async () => {
        const response = await axios.get(`${API_URL}/secuencias/all`)

        setMultitracks(response.data)
        setData({
            headers: [
                { name: 'id' }, { name: 'nombre' }, { name: 'álbum' }, { name: 'artista' },
                { name: 'enlace' }
            ],
            data: response.data
        })
    ...
    const handleChangeSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        newData.data = search === ''
            ? multitracks
            : newData.data = data.data.filter((multitrack: Multitrack) =>
                multitrack.songId.toLowerCase().includes(search.toLowerCase()) ||
                multitrack.name.toLowerCase().includes(search.toLowerCase()) ||
                multitrack.album.name.toLocaleLowerCase().includes(search.toLowerCase()) ||
                multitrack.artist.name.toLocaleLowerCase().includes(search.toLowerCase())
            )

        setData(newData)
    }

    const handleOpenModal = (d: Multitrack, type: string) => {
        setSelectedData(d)
    ...
    return (
        <>
            <Head>
                <title>
                    Secuencias | Panel Recurso Iglesia
                </title>
            </Head>
            <Box
                component="main"
            ...
            >
                <Container maxWidth={false}>
                    <Toolbar title="Secuencias" handleChange={handleChangeSearch} handleOpenCreateModal={handleOpenCreateModal} />
                    <Box sx={{ mt: 3 }}>
                        {data &&
                        ...
                        <TableBody>
                            {data.data.slice(page * limit, page * limit + limit).map((d) => (
                                <TableRow hover key={d.id} selected={selectedCustomerIds.indexOf(d.id) !== -1}>
                                    ...
                                    <TableCell style={{ width: 150 }} component="th" scope="row">
                                        <Chip label={`${d.shortener ? 'Generado' : 'Pendiente'}`} color={`${d.shortener ? 'primary' : 'warning'}`} />
                                    </TableCell>
                                    <TableCell style={{ width: 50 }} component="th" scope="row">
                                        <Stack direction="row">
